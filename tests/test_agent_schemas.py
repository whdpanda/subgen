...
def test_run_subgen_pipeline_tool_asr_cpu_fp16_guard(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    import subgen.agent.tools.run_subgen_pipeline_tool as pt
    ...
    def _fake_run_pipeline(cfg: Any) -> Any:
        seen_cfg["asr_device"] = getattr(cfg, "asr_device", None)
        seen_cfg["asr_compute_type"] = getattr(cfg, "asr_compute_type", None)
    ...
    out = pt.run_subgen_pipeline_tool(
        video_path=str(tmp_path / "dummy3.mp4"),
        asr_device=" CPU ",
        asr_compute_type=" FP16 ",
    )

    assert out["ok"] is True
    assert seen_cfg["asr_device"] == "cpu"
    assert seen_cfg["asr_compute_type"] == "int8"
